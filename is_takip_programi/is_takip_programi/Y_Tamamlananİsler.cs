...
{
    public partial class Y_Tamamlananİsler : Form
    {
        SqlConnection con;
        SqlDataAdapter da;
        SqlCommand cmd;
        ...
        public Y_Tamamlananİsler()
        {
            InitializeComponent();
        }

        private void Y_Tamamlananİsler_Load(object sender, EventArgs e)
        {
            string metin = "select * from Personel where KullaniciAdi='" + GirisEkrani.kullanicimsession + "'";
            con = new SqlConnection(SqlCon);
            da = new SqlDataAdapter("select * from Isler where Durumu='" + Convert.ToString("Tamamlandı") + "' ", con);
            ds = new DataSet();
            con.Open();
            da.Fill(ds, "Isler");
