...
{
    public partial class Y_DevamEdenİsler : Form
    {
        SqlConnection con;
        SqlDataAdapter da;
        SqlCommand cmd;
        ...
        public Y_DevamEdenİsler()
        {
            InitializeComponent();
        }

        private void Y_DevamEdenİsler_Load(object sender, EventArgs e)
        {
            string metin = "select * from Personel where KullaniciAdi='" + GirisEkrani.kullanicimsession + "'";
            con = new SqlConnection(SqlCon);
            da = new SqlDataAdapter("select * from Isler where Durumu='" + Convert.ToString("Devam Ediyor") + "' ", con);
            ds = new DataSet();
            con.Open();
            da.Fill(ds, "Isler");
